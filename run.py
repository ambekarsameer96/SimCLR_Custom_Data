...
def main(argv):
  ...
  #load numpy array and  creata a tf dataset here   
  X_train = np.zeros((10,256,256, 3), dtype=np.float32)
  y_train = np.random.randint(7, size=10)

  #create a dataset object now 
  builder = tf.data.Dataset.from_tensor_slices((X_train,y_train ))
  del X_train, y_train
  num_train_examples = 27866
  num_eval_examples = 27866
  num_classes = 14
