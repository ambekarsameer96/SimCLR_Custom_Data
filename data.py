...
def build_input_fn(builder, is_training):
  ...
  def _input_fn(params):
    ...
    #create dataset here 
    '''X_train = np.zeros((2000,32,32, 3), dtype=np.float32)
    y_train = np.random.randint(7, size=2000)
    
    dataset = tf.data.Dataset.from_tensor_slices((X_train,y_train ))'''
    #load npy file 
    img_paths = np.load('/content/drive/My Drive/medical_data/img_paths_train_gcp.npy')
    y_train = np.random.randint(15, size=27866)
    #passthese to tf dataset 
    dataset = tf.data.Dataset.from_tensor_slices((img_paths, y_train))
    ...
    #this gives image and pseudo labels
    print('-----------Dataset 1 parsed')
    #dataset = builder
    if is_training:
